...
- 2026-08-20（Asia/Shanghai）：将 Dokploy Web 部署、环境变量、端口/域名、Paper 验收和状态卷边界写入 `docs/Dokploy部署指南.md`，并从 README 增加入口；文档明确当前 `STATE_DIR` 尚未实现，未修改运行代码。

## 当前任务

- `codex/grid-risk-gate` 已完成并推送；当前已接入单账户登录与首页五列布局，并同步更新 Dokploy/使用/安全文档。网格数学、模式方向和成交后相邻反向补单保持冻结，`src/grid.js` 未修改。Nado 仍因协议不支持 reduce-only maker、且当前适配器没有接入可验证的成交历史查询，现有长/短网格 live 不能宣称完全等价支持。
...
- 仪表盘能启动/停止交易、修改 `.env`，现已接入默认 `jaychougo@gmail.com` 的单账户登录；非回环部署必须配置密码哈希、HTTPS 和可信反向代理，不能只依赖登录页。
- 本地回环地址默认允许 HTTP 以保持 `http://127.0.0.1:8283` 启动方式；这不是公网安全配置，远程访问必须使用 HTTPS，不能把本地 HTTP 例外扩展到非回环监听。
...
- 部署文档涉及持久卷时，必须先核对实际写入路径和写入方式；原子重命名的单文件状态不能简单等同于 Dokploy File Mount，优先使用独立状态目录卷。
- 单账户密码不能以明文配置；`AUTH_PASSWORD_HASH` 缺失时本地仅提示而不放行登录，非回环部署应在服务启动前 fail-closed。
