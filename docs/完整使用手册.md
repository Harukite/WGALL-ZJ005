...
`paper` 使用程序内部的模拟余额、模拟持仓和模拟订单，不会向交易所发送真实订单。程序会尽量读取交易所公开实时行情；行情接口不可用时，部分适配器可能退回合成行情。每个交易所的模拟账户各自以 `PAPER_BALANCE` 为初始余额，因此十个交易所都开模拟盘时，不代表你真实拥有十倍资金。
...
| 字段 | 默认值 | 格式与含义 |
|---|---:|---|
| `HOST` | `127.0.0.1` | 本地监听地址。保持默认最安全；非回环部署必须使用 HTTPS。 |
| `PORT` | `8283` | `1-65535` 的整数。被占用时可改为 `8284` 等未占用端口。 |
| `AUTH_EMAIL` | `jaychougo@gmail.com` | 唯一登录账户邮箱；不支持注册或多用户。 |
| `AUTH_PASSWORD_HASH` | 空 | `npm run auth:hash` 生成的 scrypt 哈希，不能填写明文密码。公网部署必须配置。 |
| `AUTH_REQUIRE_HTTPS` | `auto` | `auto` 在非回环监听时自动要求 HTTPS；公网建议显式写 `1`。 |
| `AUTH_TRUST_PROXY` | `0` | 仅在可信反向代理已终止 HTTPS 时写 `1`，用于识别 `X-Forwarded-Proto`。 |
| `PAPER_BALANCE` | `10000` | 每个模拟交易所各自的初始 USDC 余额，必须大于 0。 |

访问地址由 `HOST` 和 `PORT` 组成，例如 `http://127.0.0.1:8283`。登录页只允许
配置的唯一账户进入；默认邮箱是 `jaychougo@gmail.com`，程序没有注册入口。
...
将命令输出的完整 `AUTH_PASSWORD_HASH=...` 写入 `.env`。密码以 scrypt 哈希
保存，不会写入 URL、日志或 `.state.json`；非回环部署必须通过 HTTPS 登录。
...
可用字段：`GLOBAL_PROXY`、`DECIBEL_PROXY`、`EXTENDED_PROXY`、`RISEX_PROXY`、`ARCUS_PROXY`、`LIGHTER_PROXY`、`N1_PROXY`、`PHOENIX_PROXY`、`PHOENIX2_PROXY`、`NADO_PROXY`、`POPDEX_PROXY`。

实际优先级为全局代理，其次依次取第一个有值的交易所代理。Node.js 的全局 `fetch` 使用同一个 dispatcher，因此一个进程无法保证十个交易所分别走十个不同代理。严格隔离时应复制多份程序、使用不同 `PORT`，每份只启用一个交易所和一个代理。
...
## 12A. N1、Phoenix、Phoenix2、Nado、PopDEX 配置

五个新增交易所默认均为 `paper`，并复用现有 `GridBot` 的启动、停止、撤单确认、平仓和恢复接口。实盘前必须先以 paper 模式验证市场、精度和网格参数；适配器不会修改 `src/grid.js` 或 `src/bot.js`。

```dotenv
N1_MODE=paper
N1_KEYPAIR_PATH=secrets/id.json
N1_TRADING_ARMED=

PH_MODE=paper
PHOENIX_PRIVATE_KEY=
PHOENIX_KEYPAIR_PATH=secrets/phoenix.key
PHOENIX_SYMBOL=

PH2_MODE=paper
PHOENIX2_PRIVATE_KEY=
PHOENIX2_KEYPAIR_PATH=secrets/phoenix2.key
PHOENIX2_SYMBOL=

NADO_MODE=paper
NADO_NETWORK=ink-mainnet
NADO_PRIVATE_KEY=
NADO_KEY_PATH=
NADO_SUBACCOUNT=
NADO_PRODUCT_ID=2
...
N1 需要 Solana keypair，且只有显式设置 `N1_TRADING_ARMED=YES` 才允许真实写操作；Phoenix/Phoenix2 使用独立 Solana keypair，`PHOENIX_SYMBOL`/`PHOENIX2_SYMBOL` 可限定市场；Nado 使用 Ink 账户和 subaccount，支持 `ink-mainnet`/`ink-testnet`，但其协议只允许 taker reduce-only，因此无法安全表达当前网格的 reduce-only maker 腿，适配器会拒绝这类 live 限价单；PopDEX 使用 EVM 私钥并通过订单索引器确认链上订单。五者的 live 适配器都在缺少凭据、账户快照或权威订单确认时保持失败关闭。
...
顶部显示十个交易所的 `PAPER/LIVE` 模式、连接点、代理状态、本地后端连接和时间。
...
十个交易所分别拥有独立控制台 Tab，统一使用相同的布局和交互：市场与趋势、智能填充、网格类型、风险预览、启动/停止、调整区间、撤单、补格、未托管持仓处置、账户状态、价格/网格图、成交记录和运行日志。所有按钮最终调用同一套网格 REST 接口。
...
启动网格默认启用外围风险闸门。它不改等差网格、成交后的相邻反向补单或三种网格模式，只在下单入口和运行状态外层限制新增风险：

- **正常**：不干预现有网格；
- **软保护**：连续确认轻微单边行情、持仓达到上限、账户回撤预警或强平距离预警时，暂停对应逆势开仓并撤销已确认的逆势开仓挂单；平仓和 `reduce-only` 订单仍允许；
- **紧急保护**：短时价格冲击、账户回撤或强平距离达到紧急阈值时，锁定风险状态，执行现有的撤单确认、平仓确认和停止流程；
- **稳定恢复**：行情稳定后解除软保护，但已撤销的逆势开仓单不会自动重铺，需要人工确认后使用补格。紧急保护不会自动解除。

默认阈值为：最大持仓约为网格格数的 35%（至少 2 格数量）、5 分钟价格移动 0.75% 进入软保护、2% 进入冲击熔断；强平距离 8%/4%、账户回撤 5%/10% 分别为预警/紧急阈值。它们是保守的默认值，不是交易所统一标准，也不能保证避免强平；实盘仍必须核对交易所实际强平价、保证金、资金费、滑点和网络延迟。若交易所持仓接口不返回可验证的强平价，强平距离指标会保持未知，不会臆造价格；此时仍应依靠最大持仓、回撤闸门和交易所原生风控，并在上线前补做该交易所的保证金核验。

开发者可以在 `POST /api/{前缀}/start` 请求中传入可选的 `riskGuard` 对象覆盖阈值，例如 `{"enabled":true,"softMovePct":0.01,"shockMovePct":0.03,"maxPositionBase":2}`。不传时使用上述默认值；参数中的 `softMovePct`、`shockMovePct`、距离和回撤字段使用小数比例（`0.01` 表示 1%）。
...
- **风控哨兵**：检查十所健康、挂单同步、保证金、出区间和告警；严重/警告结果可推送。
...
服务只应在可信本机使用。交易所前缀为 `de`、`ex`、`rs`、`ar`、`lr`、`n1`、`ph`、`ph2`、`na`、`pd`。
...
| 方法 | 路径 | 作用 |
|---|---|---|
| GET | `/api/overview` | 十所摘要 |
| GET | `/api/overview/stream` | 每秒一次的十所完整 SSE 状态 |
...
- `src/exchange/`：十个交易所的 live/paper 适配器；
