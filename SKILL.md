...
- 仪表盘是单账户访问：默认邮箱为 `jaychougo@gmail.com`，密码只接受 `AUTH_PASSWORD_HASH` 的 scrypt 哈希；非回环部署要求 HTTPS，可信反向代理才可设置 `AUTH_TRUST_PROXY=1`。
- 现有源码以中文注释/用户提示为主，使用 2 空格缩进、单引号、分号；新增代码应保持就地风格，避免格式化无关行。

## 硬规

- 这是交易软件：第一次使用必须全 `paper`；`live` 会发送真实订单，任何改动都要优先验证撤单、平仓、断线与恢复安全。
- 服务默认只监听回环地址；仪表盘虽有单账户登录，非回环部署仍必须使用 HTTPS、可信反向代理和网络访问控制。
...
- `node test/auth.test.js`：本次通过；覆盖 scrypt 哈希校验、单账户登录、会话 Cookie、HTTPS 约束和未配置哈希拒绝。
...
- `node test/risk-gate.test.js`：本次通过；覆盖正常震荡、轻微单边软闸、极端冲击紧急闸、断行情跳变、最大持仓投影、回撤、强平距离、稳定恢复、紧急快照恢复、在途批量下单、GridBot 集成和 Paper 撮合竞态。
...
- 2026-08-20：策略边界回归确认：`src/grid.js` 只负责等差档位、模式方向和成交后相邻反向档位；`src/bot.js` 负责生命周期、风控、权威对账和出区间处理；EMA/ATR 趋势模块只生成推荐与参数，不应被误解为自动改网格或自动下单。
- 2026-08-20：外围风险闸门必须保持在 `GridBot` 开仓入口和生命周期外层，不得改动 `buildGrid`、`seedOrders`、`replacementFor` 或统计口径；软闸撤销逆势开仓后不自动重挂，避免与原有补单语义耦合。
- 2026-08-20：Paper 撮合遍历订单快照时，每一项都要重新校验当前订单簿；成交回调可能同步撤销同一跳跃中的后续订单，不能继续模拟已删除委托。
- 2026-08-20：风险闸门的持仓投影用于手动/补格批次和在途竞态复核；启动时仍保留原有完整网格铺单，实际成交后再按持仓上限软保护，避免把正常初始化误判成已成交敞口。
- 2026-08-20：Dokploy 部署文档必须区分“容器可运行”和“状态可持久化”；当前 `.state.json` 固定写在项目根目录，未实现 `STATE_DIR` 前不能把 `/data` 卷配置描述成已生效。
- 2026-08-20：单账户认证必须只接受 scrypt 哈希；本地回环可保留 HTTP 启动兼容，非回环默认要求 HTTPS，可信反向代理需显式声明，且缺少密码哈希时公网服务必须 fail-closed。
