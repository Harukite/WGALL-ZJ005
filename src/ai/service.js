...
const EXNAMES = {
  de: 'Decibel', ex: 'Extended', rs: 'RISEx', ar: 'Arcus', lr: 'RHC Lighter',
  n1: 'N1', ph: 'Phoenix', ph2: 'Phoenix2', na: 'Nado', pd: 'PopDEX',
};
const EXCHANGE_KEYS = ['de', 'ex', 'rs', 'ar', 'lr', 'n1', 'ph', 'ph2', 'na', 'pd'];
...
class AiService {
  constructor(bots, exchanges) {
    this.bots = bots;             // exchange key -> GridBot
    this.exchanges = exchanges;   // exchange key -> adapter
  ...
  async runSentinel() {
    ...
    try {
      ...
      const text = await aiChat({
        ...
        system: [
          '你是十交易所网格交易机器人的风控值守 AI。根据状态快照，对每个交易所分别给出巡检结论，并给一句整体结论。',
          ...
          '"per":{"de":{"level":"ok|warn|critical","summary":"该所结论(中文,50字内)","advice":"操作建议(中文,无则空)"},"ex":{...},"rs":{...},"ar":{...},"lr":{...},"n1":{...},"ph":{...},"ph2":{...},"na":{...},"pd":{...}}}',
        ].join('\n'),
  ...
  async runMarketAnalysis() {
    ...
    try {
      ...
      for (const key of ['ex', 'de', 'rs', 'ar', 'lr', 'n1', 'ph', 'ph2', 'na', 'pd']) {
        const ex = this.exchanges[key];
  ...
  async chatControl(message, history = []) {
    ...
    const text = await aiChat({
      ...
      system: [
        ...
        'action.exchange 取 de|ex|rs|ar|lr|n1|ph|ph2|na|pd。一次最多提议一个 action；用户没有明确要操作时 type 用 none。',
    ...
    if (j.action.type !== 'none' && !EXCHANGE_KEYS.includes(j.action.exchange)) j.action = { type: 'none' };
