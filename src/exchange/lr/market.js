...
export function parseMarkets(data) {
  ...
  for (const raw of rows) {
    ...
    out.push({
      ...
      status: raw.status,
      lastPrice,
      stepSize: decimalUnit(sizeDecimals),
      stepPrice: decimalUnit(priceDecimals),
      sizeDecimals,
      priceDecimals,
      minOrderSize: Number(raw.min_base_amount || decimalUnit(sizeDecimals)),
...
function decimalUnit(decimals) {
  return Number(`1e-${Number(decimals)}`);
}
