...
// 因 Node.js fetch 使用统一全局 dispatcher，当多个交易所代理不同时，
 ...
 * 优先级：全局代理 > 已配置交易所代理（按配置顺序）
 * 若各交易所代理不同，警告用户；若需独立代理，建议分开部署。
 */
export async function setupProxies(cfg) {
  const { globalProxy, de, ex, rs, ar, lr, n1, ph, ph2, na, pd } = cfg;
  ...
  // 选出实际要设置的代理（全局 > 各交易所 > Windows 手动系统代理）。
  // The system proxy is a fallback only: an explicit .env setting always wins.
  const configured = proxies.global || proxies.de || proxies.ex || proxies.rs || proxies.ar || proxies.lr
    || proxies.n1 || proxies.ph || proxies.ph2 || proxies.na || proxies.pd;
  ...
  if (!effective) return {
    de: null, ex: null, rs: null, ar: null, lr: null,
    n1: null, ph: null, ph2: null, na: null, pd: null,
    used: null, source,
  };

  // 检查是否有不同的代理
  const uniqueProxies = new Set(Object.entries(proxies)
